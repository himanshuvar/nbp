...
public class OperationSystemUtils {
    ...
    private static final Logger logger = LogManager.getLogger(OperationSystemUtils.class);
    ...
    public static boolean isFreeDiskSpace() {
        ...
        try {
            ...
            if (freeSpace < DISKSPACE_THRESHOLD) {
                logger.error("Space is not enough! Free disk space is: " + freeSpace);
                return true;
            }
            return false;
        }
        catch (Exception urle) {
            logger.error("File to url error", urle);
            return false;
        }
    }
}
